/**
 * @filename server.go
 * @description Skynet's DNS Seed server for initial peer discovery.
 *
 * @details The DNS Seed servers are used to discover peers when a node is first started.
 *          The list of allowed servers is hardcoded on a whitelist maintained by the Skynet team;
 *          new nodes must fetch the list of servers uppon startup and connect to one of them.
 *
 *          The DNS Seed servers offer a JSON-RPC 2.0 API that allows nodes to register themselves
 *          and to fetch a list of peers, which are sorted by relevance in the network given their
 *          proximity to the requesting node and it's overall reputation.
 *
 *          JSON-RPC 2.0 API:
 *          - register(hostname, ip, port, pubkey, signature) -> uuid
 *          - getPeers(uuid, signature) -> [peer]
 *          - ping(uuid, signature) -> bool
 *          
 *          The `signature` parameter is used to verify the authenticity of the request.
 *
 *          For the `register` method, the signature is calculated as follows:
 *          - Concatenate the `hostname`, `ip` and `port` parameters
 *          - Hash the resulting string using SHA-256
 *          - Sign the hash using the EC secp256k1 private key corresponding to the `pubkey` parameter
 *          - Encode the signature in base64
 *
 *          For the `getPeers` and `ping` methods, the signature is calculated as follows:
 *          - Hash the `uuid` parameter using SHA-256
 *          - Sign the hash using the EC secp256k1 private key corresponding to the `pubkey` parameter
 *          - Encode the signature in base64 
 *
 * @date 2023-10-6
...
const (
      MAX_HOSTNAME_LENGTH = 255			/** Maximum length of the hostname */
	MAX_PEERS = 100					/** Maximum number of peers to return in a single request */
	GEO_IP_DATABASE = "GeoIP2-City.mmdb"	/** Path to the GeoIP database */
	DISTANCE_WEIGHT = 0.6				/** Weight of the distance in the peer sorting algorithm */
	LAST_SEEN_WEIGHT = 0.4				/** Weight of the last seen time in the peer sorting algorithm */
	PEER_SWEEP_INTERVAL = 5 * time.Minute	/** Interval at which the peers are swept to remove the offline ones */
)
...
func handle(w http.ResponseWriter, r *http.Request) {
      /** Extract the JSON-RPC request from the http packet */
      ...
      case "get_peers":
            log.Info().Msg("Handling get_peers request from " + getIP(r))
            peers := buildPeerList()
            w.Header().Set("Content-Type", "application/json")
            json.NewEncoder(w).Encode(peers)
            log.Info().Msg("Handled get_peers request from " + getIP(r) + " | " + len(peers) + " peers returned")
      case "register":
            log.Info().Msg("Handling register request from " + getIP(r))
            var peer Peer
            err := json.Unmarshal(request.Params, &peer)
 ...
 *
 * Usage: ./seed_server -port=8080 -host=localhost -db=peers.db
 *
